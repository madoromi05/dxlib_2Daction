    ...
    : m_isDebugMode(true)
{
    ...
    m_collisionTable[4] = 1;
    m_collisionTable[0] = 1;
    m_collisionTable[41] = 2;
...
bool TileLayer::IsWall(float x, float y) {
    ...
    int type = m_collisionTable[id];

    if (type == 0)return false; //空気
    if (type == 1)return true;  //壁

    int localX = static_cast<int>(x) % stage_information::kMapGridSize;
    int localY = static_cast<int>(y) % stage_information::kMapGridSize;
